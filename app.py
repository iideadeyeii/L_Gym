...
def run_captioning(images, concept_sentence, *captions):
    ...
    try:
        # Try to import CLIP for image understanding
        from PIL import Image
        import torch
        ...
        contexts = [
            "a person portrait", "a landscape", "wildlife", "a cat", "a dog", "food", 
            "architecture", "cityscape", "abstract art", "interior scene", "flowers",
            "vehicle", "clothing", "technology", "beach scene", "night sky", "mountains",
            "forest", "river", "underwater scene", "sports activity", "closeup of face",
            "group of people", "product photography"
        ]
        
        context_inputs = clip.tokenize(contexts).to(device)
        ...
        for i, image_path in enumerate(images):
            ...
            # Skip text files
            if isinstance(image_path, str) and image_path.endswith('.txt'):
                continue
            ...
            if isinstance(image_path, str):  # If image is a file path
                ...
                with torch.no_grad():
                    ...
                    text_features = model.encode_text(context_inputs)
                    ...
                    # Get top 3 contexts
                    top_contexts = [contexts[idx] for idx in indices]
                    
                # Build descriptive caption
                file_name = os.path.basename(image_path)
                context_description = ", ".join(top_contexts)
                base_caption = f"A detailed image showing {context_description}"
                
                # Add concept sentence if provided
                caption_text = f"{concept_sentence}, {base_caption}" if concept_sentence else base_caption
                captions[i] = caption_text
                print(f"Generated context-aware caption for {file_name}: {caption_text}")
            
            # Yield after each image is processed
            yield captions
        
        # Final yield to ensure all captions are returned
        yield captions
        
    except Exception as e:
        print(f"Error in context-aware captioning: {e}")
        ...
        for i, image_path in enumerate(images):
            ...
            # Generate simple caption with more variation
            file_name = os.path.basename(image_path) if isinstance(image_path, str) else f"image_{i+1}"
            
            # Generate varied base captions
            import random
            base_templates = [
                "A detailed image showing {}",
                "A high-quality photograph of {}",
                "A professional picture featuring {}"
            ]
            
            subjects = [
                "a subject", "interesting elements", "visual content", 
                "photographic composition", "artistic elements"
            ]
            
            base_caption = random.choice(base_templates).format(random.choice(subjects))
            
            # Add concept sentence if provided
            caption_text = f"{concept_sentence}, {base_caption}" if concept_sentence else base_caption
            captions[i] = caption_text
            print(f"Generated simple caption for {file_name}: {caption_text}")
    ...
    with gr.Tabs() as tabs:
        ...
        """, elem_classes="group_padding")
                    ...
                    with gr.Group(visible=False) as captioning_area:
                        do_captioning = gr.Button("Add context-aware captions")
                        gr.Markdown("""
                        ℹ️ **Context-Aware Captioning**
                        
                        This will analyze your images using OpenAI's CLIP model to generate captions based on visual content.
                        
                        The system will:
                        1. Detect the main subjects and context in each image
                        2. Generate appropriate descriptive text
                        3. Include your trigger word in all captions
                        
                        *First upload will be slower as CLIP model needs to be downloaded (295MB)*
                        """)
